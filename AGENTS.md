...
- Estratégia Concursos — **limite de 3 produtos matriculados por vez**. Curso/pacote não matriculado não abre: em `/app/dashboard/assinaturas`, clicar `DESMATRICULAR` no que vai sair e digitar `CORUJA`, depois buscar o produto desejado na aba PACOTES, clicar `MATRICULAR` e digitar `CORUJA` de novo, e recarregar. **A palavra `CORUJA` vale nos dois sentidos, matrícula e desmatrícula.** É reversível, e **qualquer pacote pode entrar ou sair do rodízio** — inclusive o da PRF — quando a tarefa pedida pelo Elvis exigir. Antes de desmatricular, checar placeholders `.txt` (aulas ainda não publicadas) daquele pacote na pasta do Drive e avisar o Elvis. **Ao procurar um produto no catálogo, buscar sempre por PACOTE** (`type=pacote` na busca da API, aba PACOTES na tela): o que o Elvis chama de "curso" é um pacote no Estratégia — as disciplinas dentro dele têm outro nome (ex: pacote "Curso Regular para Área Fiscal" contém "Concursos da Área Fiscal - Curso Básico de Direito Administrativo") (confirmado 2026-08-20).
